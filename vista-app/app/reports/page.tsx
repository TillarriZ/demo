...
const MOCK_REPORTS: Report[] = [
  {
    ...
    title: "Human Factor",
    date: "10.02.2025",
    situationSummary: "Участились задержки рейсов и сбои в коммуникации между OCC и наземными службами. Нехватка диспетчеров в пиковые часы и усталость экипажей приводят к повторяющимся инцидентам и росту стоимости потерь.",
  ...
  },
  {
    ...
    title: "Human Factor",
    date: "28.01.2025",
    situationSummary: "На складе зафиксированы почти-столкновения и наезд техники при разгрузке. Отсутствуют чёткие правила передвижения людей и техники, spotter-роли не закреплены, что повышает риски в зоне погрузки.",
    ...
    processGaps: ["Нет выделенных пешеходных зон и регламентированного времени передвижения на складе; spotter-роли не закреплены", "Редкое обновление карты рисков в зоне погрузки"],
    ...
    hypotheses: ["Введение geo-alerts на карте склада снизит риск наездов", "Закрепление spotter-ролей и выделенных пешеходных зон с регламентом передвижения уменьшит почти-столкновения"],
    recommendations: ["Внедрить geo-alerts на карте склада", "Закрепить spotter-роли и выделенные пешеходные зоны с регламентом передвижения в SOP", "Улучшить освещение и разметку в зоне погрузки", "Обучение подрядчиков по процедурам при смене бригад"],
  },
  {
    id: "3",
    title: "Human Factor",
    date: "15.01.2025",
    situationSummary: "При задержках рейсов более часа растёт число жалоб на багаж и задержку его подачи. Регистрация не синхронизирована с реальным прибытием пассажиров, что создаёт сбои в логистике багажа.",
        ...
        <Card>
          ...
          <CardBody className="pt-0 space-y-6">
            <section className="pb-4 border-b border-slate-200">
              <h3 className="text-sm font-semibold text-slate-700 mb-2">Ситуация и проблема</h3>
              <p className="text-slate-600 text-sm">{selected.situationSummary}</p>
            </section>
            <section>
